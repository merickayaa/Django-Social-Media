...
import random
from django.db.models import Aggregate, Prefetch
...
def index(request):
    ...
    feed_list = Post.objects.filter(user__username__in=user_following_list).annotate(liker_ids_str = GroupConcat('likes__user')).prefetch_related(Prefetch('comments', Comment.objects.select_related('user')))
    ...
    return render(request, 'platform.html', {'user_following':user_following,'user_profile':user_object, 'posts':feed_list,'suggestions_username_profile_list':suggestions_username_profile_list[:4]})
...
def dashboard(request, user_slug):
    ...
    followerCount = len(Follower.objects.filter(user=profile))
    postCount = len(posts)
    followingCount = len(Follower.objects.filter(follower=profile))
    if Follower.objects.filter(follower=follower, user=profile):
...
def posts(request,user_slug):
    profile = User.objects.get(slug=user_slug)
    # posts = Post.objects.filter(user=profile).values('id', 'caption','', 'likes__user')
    # for post in posts:
    #     post_user_likes = LikePost.objects.filter(post=post).values_list("user", flat = True)
    #     post.userlikes = post_user_likes
    #     print("post_user_likes: ", post_user_likes)
    #     print("post: ", post.userlikes)
    
    # posts = Post.objects.filter(user=profile).select_related("user").prefetch_related('likes')
    # for post in posts:
